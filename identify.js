...
Berikan jawaban Anda HANYA dalam format JSON yang valid (tanpa markdown backticks) dengan struktur PERSIS seperti ini:
{
    ...
    "references": [
        {
            "citation": "Kutip minimal 3 artikel jurnal ilmiah open access nyata yang membahas tumbuhan ini berformat APA Style.",
            "doi_link": "https://doi.org/..."
        },
        {
            "citation": "Referensi ke-2 berformat APA Style",
            "doi_link": "https://doi.org/..."
        },
        {
            "citation": "Referensi ke-3 berformat APA Style",
            "doi_link": "https://doi.org/..."
        }
    ]
}
...
- Berikan MINIMAL 3 referensi jurnal ilmiah open access yang relevan dalam FORMAT APA STYLE (7th edition) dengan DOI link aktif.
- Setiap referensi harus memiliki field "citation" dalam format: Penulis, A. A. (Tahun). Judul artikel. Nama Jurnal, Volume(Issue), halaman-halaman. https://doi.org/...
    ...
    if (data.references && data.references.length > 0) {
        ...
        data.references.forEach(ref => {
            if (ref.citation) {
                ...
                refsHtml += `<li>${ref.citation} ${ref.doi_link ? `<br><a href="${ref.doi_link}" target="_blank" rel="noopener">${ref.doi_link}</a>` : ''}</li>`;
            } else {
                // Fallback for old format
                const authors = ref.authors ? `${ref.authors}. ` : '';
                refsHtml += `<li>${authors}<a href="${ref.doi_link}" target="_blank" rel="noopener">${ref.title}</a></li>`;
            }
        });
        refsHtml += `</ul>`;
    } else {
